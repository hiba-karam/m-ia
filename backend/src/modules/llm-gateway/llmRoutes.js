...
const { callKimi } = require('./adapters/kimiAdapter');
...
router.post('/chat', tokenGuard, async (req, res) => {
    ...
    try {
        ...
        if (provider.name === 'Claude') {
            ...
            aiResponse = await callDeepSeek(provider, systemInstruction, prompt);
        } else if (provider.name === 'Kimi') {
            aiResponse = await callKimi(provider, systemInstruction, prompt);
        } else {
